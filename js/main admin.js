...
function processFile(e) {
  ...
  var mySets = report.mySets,
    myProfiles = report.myDetailSetsForProfile,
  ...
  for (var i = 0; i < mySetsKeys.length; i++) {
    if (mySets[mySetsKeys[i]].sets > 0) {
      $("#AppidsTable").append(
        $(
          `<tr class='item'><td>${mySets[mySetsKeys[i]].name}</td><td>${
            mySetsKeys[i]
          }</td><td class="sets">${
            mySets[mySetsKeys[i]].sets
          }</td><td class='TFrate'>${
            mySets[mySetsKeys[i]].TFrate
          }</td><td class='CSrate'>${mySets[mySetsKeys[i]].CSrate}</td><td>${(
            mySets[mySetsKeys[i]].sets / mySets[mySetsKeys[i]].TFrate
          ).toFixed(2)}</td><td>${(
            mySets[mySetsKeys[i]].sets / mySets[mySetsKeys[i]].CSrate
          ).toFixed(2)}</td><td><input type="number" class="cart" data-id="${
            mySetsKeys[i]
          }" placeholder="0" max="${
            mySets[mySetsKeys[i]].sets
          }" min="0"></td><td>${
            targetSets[mySetsKeys[i]] ? targetSets[mySetsKeys[i]].sets : 0
          }</td><td><input type="number" class="target" data-id="${
            mySetsKeys[i]
          }" placeholder="0" min="${
            targetSets[mySetsKeys[i]] ? targetSets[mySetsKeys[i]].sets : 0
          }" max="${
