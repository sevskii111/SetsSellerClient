...
function processFile(e) {
    ...
    var myNormals = foil ? report.myFoils : report.myNormals,
        targetNormals = foil ? report.targetFoils : report.targetNormals,
        //rates = report.rates,
        type;
    $("#Import").addClass("d-none");
    $("#TableContainer").removeClass("d-none");
    for (var card in myNormals) {
        $("#AppidsTable").append($(
            `<tr class='item'><td>${card}</td><td class="sets">${myNormals[card]}</td><td><input data-normal="true" type="number" class="cart" data-id="${card}" placeholder="0" max="${myNormals[card]}" min="0"></td><td>${targetNormals[card]}</td><td><input data-normal="true" type="number" class="target" data-id="${card}" placeholder="0" min="${targetNormals[card]}" max="${myNormals[card] + targetNormals[card]}"></td></tr>`
        ));
    }
